...
with col_in:
    ...
    # Dados reais de collections_df (mantendo código original)
...
with col_cal:
    st.markdown("<div class='section-card'>", unsafe_allow_html=True)
    st.markdown("<h3 style='font-size:1.5rem; font-family:Inter, sans-serif;'>📅 Payment Calendar</h3>", unsafe_allow_html=True)
    ...
    if 'payments' not in st.session_state:
        if os.path.exists(csv_path):
            try:
                ...
                if not df.empty and set(["Date", "Department", "Vendor", "Amount (€)", "Notes"]).issubset(df.columns):
...
Valor: {float(pay['Amount (€)']):.0f}€
Notas: {pay.get('Notes', '-')}
"""
        events.append({
            "id": str(i),
            "title": f"{pay['Department']} - {pay['Vendor']} ({float(pay['Amount (€)']):.0f}€)",
            "start": str(pay['Date']),
            "extendedProps": {
                "notes": pay.get('Notes', '-'),
    ...
    if st.session_state.show_add_form:
        with st.container():
            ...
            submitted = st.button("💾 Submit Payment", key="submit_button")
            if submitted:
                if vendor.strip() == "" or amount <= 0:
                    st.error("Por favor preenche o nome do fornecedor e um valor válido.")
                else:
                    new_entry = {
                        "Date": pdate,
                        "Department": dept,
