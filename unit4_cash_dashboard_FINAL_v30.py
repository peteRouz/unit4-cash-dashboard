...
with col1:
    ...
    st.markdown("<h6>Cashflow Forecast</h6>", unsafe_allow_html=True)
    # Preparação de dados
    ...
    # Cores personalizadas: Jan-Mar (verde, azul), Apr-Dec (amarelo)
    inflow_colors  = ['#4caf50', '#4caf50', '#4caf50'] + ['lightyellow'] * 9
    outflow_colors = ['#ff9800',  '#ff9800',  '#ff9800']  + ['lightyellow'] * 9

    fig_cf, ax_cf = plt.subplots(figsize=(7.5, 2.8))
    ax_cf.set_title("Cashflow Forecast", loc='center', fontsize=10, weight='bold')
    bar_w = 0.35
    ax_cf.bar(x - bar_w/2, inflow,  bar_w,
              color=inflow_colors, edgecolor='black', linewidth=0.5, label='Inflow')
    ax_cf.bar(x + bar_w/2, outflow, bar_w,
              color=outflow_colors, edgecolor='black', linewidth=0.5, label='Outflow')
    ax_cf.plot(x_s, netflow_s, color='#303f9f', linewidth=0.75, label='Net Flow')
    # Rótulos
    for i, v in enumerate(inflow):
        ax_cf.text(i - bar_w/2, v + 2000,
                   f"{v:,.0f}".replace(',', ' '), ha='center', fontsize=5)
    for i, v in enumerate(outflow):
        ax_cf.text(i + bar_w/2, v - 7000,
                   f"{v:,.0f}".replace(',', ' '), ha='center', fontsize=5)
    ax_cf.set_xticks(x)
...
with col2:
    st.markdown("<div class='section-card'>", unsafe_allow_html=True)
    st.markdown("<h6>Actual vs Forecast</h6>", unsafe_allow_html=True)
    # Carregar dados para Actual vs Forecast
...
# Converter valores numéricos e formatar
...
# --- Gerar HTML estilizado para tabela com striping leve ---
styled = collections_df.style \
    .set_table_styles([
        ...
        },
        {
            ...
            'props': [
                ('background-color', '#f8f8f8')  # leve sombreado em linhas ímpares
            ]
        },
        {
            'selector': 'tbody tr:nth-child(even)',
            'props': [
                ('background-color', 'white')   # linhas pares sem sombreado
            ]
        }
    ]) \
    .hide(axis="index")  # Esconder coluna de índices
